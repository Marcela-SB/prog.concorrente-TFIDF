                ...
                try (RandomAccessFile raf = new RandomAccessFile(FILE_PATH, "r");
                    ...
                    try (var is = Channels.newInputStream(raf.getChannel());
                        ...
                        while (bytesRead < segment.size() && (row = reader.readLine()) != null) {
                            bytesRead += row.getBytes().length + 1;
                            freq.clear();
                            uniqueTerms.clear(); // Limpa o set para a próxima linha
                            ...
                            for (String term : uniqueTerms) {
                                double tf = (double) freq.get(term) / wordCount;
                                double idf = Math.log((double) totalDocsCount / documentsWithTerm.get(term));
                                sb.append(currentLine).append(';').append(term).append(';')
        ...
        for (int i = 0; i < numThreads; i++) {
            ...
            threads2.add(Thread.ofPlatform().name("JMH-TFIDF-" + i).start(() -> {
                ...
                for (String row : subList) {
                    ...
                    for (String term : uniqueTerms) {
                        double tf = (double) freq.get(term) / wordCount;
                        double idf = Math.log((double) finalTotal / documentsWithTerm.get(term));
